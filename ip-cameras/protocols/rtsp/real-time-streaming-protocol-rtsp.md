...
\[[RFC Home](https://www.rfc-editor.org/)] \[[TEXT](https://www.rfc-editor.org/rfc/rfc2326.txt)|[PDF](https://www.rfc-editor.org/rfc/pdfrfc/rfc2326.txt.pdf)|[HTML](https://www.rfc-editor.org/rfc/rfc2326.html)] \[[Tracker](https://datatracker.ietf.org/doc/rfc2326)] \[[IPR](https://datatracker.ietf.org/ipr/search/?rfc=2326\&submit=rfc)] \[[Errata](https://www.rfc-editor.org/errata/rfc2326)] \[[Info page](https://www.rfc-editor.org/info/rfc2326)]\
\
Obsoleted by: [7826](https://www.rfc-editor.org/rfc/rfc7826) PROPOSED STANDARD\
Errata Exist
...
```
 ...
 Purpose
   ...
   media stream with the control stream is possible (see ).
...
```
   ...
   Message:
          ...
          structured sequence of octets matching the syntax defined in
           and transmitted via a connection or a
   ...
   Presentation description:
          ...
          content.  Other IETF protocols such as SDP ( [6]) use
...
```
 ...
 Protocol Properties
   ...
   Secure:
          ...
          mechanisms such as basic ( [2, ]) and
   ...
   Transport-independent:
          RTSP may use either an unreliable datagram protocol (UDP) (
           [9]), a reliable datagram protocol (RDP, , not
          widely used [10]) or a reliable stream protocol such as TCP
          ( [11]) as it implements application-level reliability.
...
```
 ...
 Extending RTSP
   ...
   A server SHOULD implement all header fields described in .
...
```
   ...
   TEARDOWN:
          ...
          field () to identify the RTSP session whose state
          is being manipulated. The server generates session identifiers
          in response to SETUP requests ().
...
```
 ...
 Notational Conventions
   ...
   to Section X.Y of the current HTTP/1.1 specification ( [2]).

   All the mechanisms specified in this document are described in both
   prose and an augmented Backus-Naur form (BNF) similar to that used in
   [H2.1]. It is described in detail in  [17], with the
...
```
   ...
   URLs ( [20]). URLs may refer to a stream or an aggregate of
   streams, i.e., a presentation. Accordingly, requests described in
    can apply to either the whole presentation or an individual
...
```
 ...
 Option Tags
   ...
   RTSP. These tags are used in Require () and Proxy-
 ...
 Registering New Option Tags with IANA
...
```
 ...
 RTSP Message
   ...
   UTF-8 encoding ( [21]). Lines are terminated by CRLF, but
...
```
 ...
 Message Length
   ...
   2.     If a Content-Length header field () is present,
...
```
 ...
 Status Code and Reason Phrase
   ...
   defined in . The Reason-Phrase is intended to give a short
...
```
 ...
 Reliability and Acknowledgements
   ...
   The round-trip time is estimated as in TCP () [18], with an
...
```
     ...
     T/TCP () [22], can be beneficial.

   The Timestamp header () is used to avoid the
   ...
   ), which is incremented by one for each distinct request
...
```
   ...
   copy of the entity). See  for further discussion of the
   expiration model.
   ...
   [H3.3]; it MUST be in -date format:
...
```
   ...
   See  for more details on the mechanics of this message
   and a usage example.
 ...
 Range
   ...
   defines the smpte (), npt (), and clock
   () range units. Within RTSP, byte ranges [H14.36.1] are
...
```
   ...
   in the Proxy-Require field instead (see ).
...
```
 ...
 Session
   ...
   the media server (see ). Once a client receives a Session
...
```
   ...
   from the same server. (See example in ). However, multiple
...
```
   ...
   interleaved:
          ...
          control stream, using the mechanism defined in .
...
```
 ...
 Unsupported
   ...
   Proxy-Require field (), if there is a proxy on the path
...
```
   ...
   described in . A cache MUST answer any DESCRIBE requests
...
```
 ...
 Playing media into an existing session
...
```
   ...
   ( [2], as of this writing), future HTTP specifications may
...
Appendix A: RTSP Protocol State Machines
   ...
   will have an effect on the states of movie/audio and movie/video.
     ...
     URLs or a relation to the filesystem. See .
...
```
   ...
   header (). If the server is in state Ready, it MAY
...
Appendix B: Interaction with RTP
   ...
   For scaling (see ), RTP timestamps should correspond to
   the playback timing. For example, when playing video recorded at 30
   frames/second at a scale of two and speed () of one, the
...
Appendix C: Use of SDP for RTSP Session Descriptions
   ...
   The Session Description Protocol (SDP,  [6]) may be used to
 ...
 Control URL
   ...
   following the rules and conventions set out in  [25].
...
```
 ...
 Payload type(s)
   ...
   payload type is a static payload type from  [1], no other
   ...
   specified in  (Sections  and ), or an experimental encoding
   with a "X-" prefix as specified in SDP ( [6]).  Codec-
   ...
   register new encodings should follow the procedure in  [1].
...
```
 ...
 Range of presentation
   ...
   The units and their values are as defined in , 3.6 and
   3.7.
 ...
 Connection Information
  ...
  C.1.8 Entity Tag
   ...
   this identifier in the If-Match field (see ) to only
...
Appendix D: Minimal RTSP implementation
 ...
 Client
   ...
   implementations and/or to be a "good citizen".
     ...
     * Understand SDP session descriptions as defined in 
...
```
 ...
 Server
   ...
   implementations and/or to be a "good citizen".
     ...
     * Generate SDP session descriptions as defined in 
...
References
   ...
   7      Franks, J., Hallam-Baker, P., and J. Hostetler, "An extension to
          HTTP: digest access authentication", , January 1997.

   8      Postel, J., "User Datagram Protocol", STD 6, , August
          1980.

   9      Hinden, B. and C. Partridge, "Version 2 of the reliable data
          protocol (RDP)", , April 1990.
...
```
   ...
   17     Braden, B., "Requirements for internet hosts - application and
          support", STD 3, , October 1989.

   18     Elz, R., "A compact representation of IPv6 addresses", 
          , April 1996.
   ...
   24     Fielding, R., "Relative uniform resource locators", ,
          June 1995.
