# Diet Pi Supported Software

## DietPi software options[¶](https://dietpi.com/docs/software/#dietpi-software-options) <a href="#dietpi-software-options" id="dietpi-software-options"></a>

### Overview[¶](https://dietpi.com/docs/software/#overview) <a href="#overview" id="overview"></a>

**DietPi-Software** is a core DietPi tool, allowing you to install **Ready to run** and **optimized software items** for your device. It automates the installation and configuration process for you and also applies specific optimizations unique to your hardware model. As a result DietPi-Software ensure you get the **maximum performance** from your device.

<details>

<summary>How do I run <strong>DietPi-Software</strong> and install <strong>optimised software</strong> items?</summary>

\


<img src="https://dietpi.com/docs/assets/images/dietpi-software.jpg" alt="DietPi-Software menu screenshot" data-size="original">

</details>

### [Advanced Networking](https://dietpi.com/docs/software/advanced\_networking/)[¶](https://dietpi.com/docs/software/#advanced-networking) <a href="#advanced-networking" id="advanced-networking"></a>

* [**WiFi HotSpot - Turn your device into a wireless hotspot/access point**](https://dietpi.com/docs/software/advanced\_networking/#wifi-hotspot)
* [**Tor HotSpot - Optional: Routes all WiFi hotspot traffic through the Tor network**](https://dietpi.com/docs/software/advanced\_networking/#tor-hotspot)
* [**HAProxy - High performance TCP/HTTP load balancer**](https://dietpi.com/docs/software/advanced\_networking/#haproxy)
* [**frp - Reverse Proxy**](https://dietpi.com/docs/software/advanced\_networking/#frp)

### [BitTorrent & Download Tools](https://dietpi.com/docs/software/bittorrent/)[¶](https://dietpi.com/docs/software/#bittorrent-download-tools) <a href="#bittorrent-download-tools" id="bittorrent-download-tools"></a>

* [**Transmission - Lightweight BitTorrent server with web interface**](https://dietpi.com/docs/software/bittorrent/#transmission)
* [**Deluge - Alternative BitTorrent server with web interface**](https://dietpi.com/docs/software/bittorrent/#deluge)
* [**qBittorrent - Lightweight and fast (C++) BitTorrent server with web interface**](https://dietpi.com/docs/software/bittorrent/#qbittorrent)
* [**rTorrent - BitTorrent server with ruTorrent web interface**](https://dietpi.com/docs/software/bittorrent/#rtorrent)
* [**Aria2 - Download manager with web interface**](https://dietpi.com/docs/software/bittorrent/#aria2)
* [**SABnzbd - NZB download manager with web interface**](https://dietpi.com/docs/software/bittorrent/#sabnzbd)
* [**Medusa - Automatically download TV shows**](https://dietpi.com/docs/software/bittorrent/#medusa)
* [**Sonarr - Automatically download TV shows**](https://dietpi.com/docs/software/bittorrent/#sonarr)
* [**Radarr - Automatically download Movies**](https://dietpi.com/docs/software/bittorrent/#radarr)
* [**Bazarr - Automatically download Subtitles for Sonarr/Radarr**](https://dietpi.com/docs/software/bittorrent/#bazarr)
* [**Lidarr - Automatically download Music**](https://dietpi.com/docs/software/bittorrent/#lidarr)
* [**Readarr - Automatically download Ebooks**](https://dietpi.com/docs/software/bittorrent/#readarr)
* [**Prowlarr - Indexer manager & proxy for PVR**](https://dietpi.com/docs/software/bittorrent/#prowlarr)
* [**Jackett - API Support for your favourite torrent trackers**](https://dietpi.com/docs/software/bittorrent/#jackett)
* [**NZBGet - NZB download manager with web interface**](https://dietpi.com/docs/software/bittorrent/#nzbget)
* [**HTPC Manager - combines all your favourite software into one slick interface**](https://dietpi.com/docs/software/bittorrent/#htpc-manager)
* [**youtube-dl - download videos from YouTube and other sites**](https://dietpi.com/docs/software/bittorrent/#youtube-dl)

### [Camera & Surveillance](https://dietpi.com/docs/software/camera/)[¶](https://dietpi.com/docs/software/#camera-surveillance) <a href="#camera-surveillance" id="camera-surveillance"></a>

* [**RPi Cam Web Interface - Web interface & controls for your RPi camera**](https://dietpi.com/docs/software/camera/#rpi-cam-web-interface)
* [**MotionEye - Web interface & surveillance for your camera**](https://dietpi.com/docs/software/camera/#motioneye)
* [**mjpg-streamer - Simple camera streaming tool with HTML plugin**](https://dietpi.com/docs/software/camera/#mjpg-streamer)

### [Cloud & Backup systems](https://dietpi.com/docs/software/cloud/)[¶](https://dietpi.com/docs/software/#cloud-backup-systems) <a href="#cloud-backup-systems" id="cloud-backup-systems"></a>

* [**ownCloud - Your own personal cloud based backup/data storage system**](https://dietpi.com/docs/software/cloud/#owncloud)
* [**Nextcloud - Self-hosted productivity platform**](https://dietpi.com/docs/software/cloud/#nextcloud)
* [**Nextcloud Talk - Video calls via Nextcloud, including TURN server**](https://dietpi.com/docs/software/cloud/#nextcloud-talk)
* [**Pydio - Feature-rich backup and sync server with web interface**](https://dietpi.com/docs/software/cloud/#pydio)
* [**UrBackup Server - Full backups for systems on your network**](https://dietpi.com/docs/software/cloud/#urbackup)
* [**Gogs - GitHub style server, with web interface**](https://dietpi.com/docs/software/cloud/#gogs)
* [**Gitea - GitHub style server, with web interface**](https://dietpi.com/docs/software/cloud/#gitea)
* [**Syncthing - Backup and sync server with web interface**](https://dietpi.com/docs/software/cloud/#syncthing)
* [**MinIO - S3 compatible distributed object server**](https://dietpi.com/docs/software/cloud/#minio)
* [**vaultwarden - Unofficial Bitwarden password manager server written in Rust**](https://dietpi.com/docs/software/cloud/#vaultwarden)
* [**FuguHub - Your Own Personal Cloud Server**](https://dietpi.com/docs/software/cloud/#fuguhub)
* [**File Browser - Light web based file manager with sharing features**](https://dietpi.com/docs/software/cloud/#file-browser)
* [**Rclone - Utility to sync your files to cloud storages**](https://dietpi.com/docs/software/cloud/#rclone)
* [**Restic - Fast, efficient and secure command-line backup program**](https://dietpi.com/docs/software/cloud/#restic)

### [Databases & Data Stores](https://dietpi.com/docs/software/databases/#overview.md)[¶](https://dietpi.com/docs/software/#databases-data-stores) <a href="#databases-data-stores" id="databases-data-stores"></a>

* [**MariaDB - Open Source Relational Database**](https://dietpi.com/docs/software/databases/#mariadb)
* [**phpMyAdmin - SQL Administration Tool for MariaDB**](https://dietpi.com/docs/software/databases/#phpmyadmin)
* [**SQLite - Small, Fast and High reliable SQL database engine**](https://dietpi.com/docs/software/databases/#sqlite)
* [**Redis - Open Source In-memory key–value Data Store**](https://dietpi.com/docs/software/databases/#redis)
* [**InfluxDB - Open Source Time Series Database**](https://dietpi.com/docs/software/databases/#influxdb)
* [**PostgreSQL - Persistent and advanced SQL database engine**](https://dietpi.com/docs/software/databases/#postgresql)

### [Desktops](https://dietpi.com/docs/software/desktop/)[¶](https://dietpi.com/docs/software/#desktops) <a href="#desktops" id="desktops"></a>

* [**LXDE - Highly optimized ultra lightweight desktop**](https://dietpi.com/docs/software/desktop/#lxde)
* [**LXQt - Slick and optimized lightweight desktop**](https://dietpi.com/docs/software/desktop/#lxqt)
* [**MATE - Popular Gnome 2 desktop**](https://dietpi.com/docs/software/desktop/#mate)
* [**Xfce - Lightweight desktop**](https://dietpi.com/docs/software/desktop/#xfce)
* [**GNUstep - Alternative/unique desktop**](https://dietpi.com/docs/software/desktop/#gnustep)

#### [Desktop Utilities](https://dietpi.com/docs/software/desktop/)[¶](https://dietpi.com/docs/software/#desktop-utilities) <a href="#desktop-utilities" id="desktop-utilities"></a>

* [**Chromium - Web browser**](https://dietpi.com/docs/software/desktop/#chromium)
* [**Firefox - Web browser**](https://dietpi.com/docs/software/desktop/#firefox)
* [**GIMP - Image editor**](https://dietpi.com/docs/software/desktop/#gimp)
* [**QuiteRSS - RSS and Atom news feeds reader**](https://dietpi.com/docs/software/desktop/#quiterss)
* [**Xfce Power Manager - Power manager with brightness control**](https://dietpi.com/docs/software/desktop/#xfce-power-manager)

### [Development & Programming](https://dietpi.com/docs/software/programming/)[¶](https://dietpi.com/docs/software/#development-programming) <a href="#development-programming" id="development-programming"></a>

* [**Python 3 - High-level interpreted programming language**](https://dietpi.com/docs/software/programming/#python-3)
* [**Go - Programming language**](https://dietpi.com/docs/software/programming/#go)
* [**Java - Programming language**](https://dietpi.com/docs/software/programming/#java)
* [**Mono - .NET Framework implementation**](https://dietpi.com/docs/software/programming/#mono)
* [**Docker - Build, ship, and run distributed applications**](https://dietpi.com/docs/software/programming/#docker)
* [**Docker-Compose - Run multi-container applications on Docker**](https://dietpi.com/docs/software/programming/#docker-compose)
* [**Portainer - Docker container management**](https://dietpi.com/docs/software/programming/#portainer)
* [**VSCodium - FLOSS version of Microsoft VSCode**](https://dietpi.com/docs/software/programming/#vscodium)
* [**Git - Distributed version control system**](https://dietpi.com/docs/software/programming/#git)

### [Distributed Projects](https://dietpi.com/docs/software/distributed\_projects/)[¶](https://dietpi.com/docs/software/#distributed-projects) <a href="#distributed-projects" id="distributed-projects"></a>

* [**Folding@Home - Use processing power to assist with disease research**](https://dietpi.com/docs/software/distributed\_projects/#foldinghome)
* [**Tor Relay - Add a node to the Tor network**](https://dietpi.com/docs/software/distributed\_projects/#tor-relay)
* [**YaCy - Decentralized open source search engine**](https://dietpi.com/docs/software/distributed\_projects/#yacy)
* [**IPFS Node - Contribute to a decentralized internet**](https://dietpi.com/docs/software/distributed\_projects/#ipfs-node)
* [**ADS-B Feeder - track airplanes using SDRs and feed the data to ADS-B aggregators**](https://dietpi.com/docs/software/distributed\_projects/#ads-b-feeder)

### [DNS Servers](https://dietpi.com/docs/software/dns\_servers/)[¶](https://dietpi.com/docs/software/#dns-servers) <a href="#dns-servers" id="dns-servers"></a>

* [**Pi-hole - Network-wide Ad Blocking**](https://dietpi.com/docs/software/dns\_servers/#pi-hole)
* [**Unbound - A validating, recursive, and caching DNS resolver**](https://dietpi.com/docs/software/dns\_servers/#unbound)
* [**AdGuard Home - A powerful network-wide ads & trackers blocking DNS server**](https://dietpi.com/docs/software/dns\_servers/#adguard-home)

### [File Servers](https://dietpi.com/docs/software/file\_servers/)[¶](https://dietpi.com/docs/software/#file-servers) <a href="#file-servers" id="file-servers"></a>

* [**ProFTPD - Simple, efficient, lightweight FTP file server**](https://dietpi.com/docs/software/file\_servers/#proftpd)
* [**Samba - Feature rich file server**](https://dietpi.com/docs/software/file\_servers/#samba)
* [**vsftpd - Feature rich FTP file server**](https://dietpi.com/docs/software/file\_servers/#vsftpd)
* [**NFS - Network file system server**](https://dietpi.com/docs/software/file\_servers/#nfs)

### [Gaming & Emulation](https://dietpi.com/docs/software/gaming/)[¶](https://dietpi.com/docs/software/#gaming-emulation) <a href="#gaming-emulation" id="gaming-emulation"></a>

* [**OpenTyrian - Open-source port of the DOS game Tyrian**](https://dietpi.com/docs/software/gaming/#opentyrian)
* [**Cuberite - Fast Minecraft server with web interface**](https://dietpi.com/docs/software/gaming/#cuberite)
* [**MineOS - Multiple Minecraft servers with web interface**](https://dietpi.com/docs/software/gaming/#mineos)
* [**Nukkit - Server for Minecraft Pocket Edition**](https://dietpi.com/docs/software/gaming/#nukkit)
* [**Amiberry - Optimised Amiga emulator for ARM-based SoCs**](https://dietpi.com/docs/software/gaming/#amiberry)
* [**DXX-Rebirth - Descent 1 and 2 OpenGL port**](https://dietpi.com/docs/software/gaming/#dxx-rebirth)
* [**Steam - Steam client**](https://dietpi.com/docs/software/gaming/#steam)
* [**PaperMC - Fast and optimized Minecraft server**](https://dietpi.com/docs/software/gaming/#papermc)
* [**Box86 - i386 userspace emulation for ARMv7**](https://dietpi.com/docs/software/gaming/#box86)
* [**Box64 - x86\_64 userspace emulation for ARMv8**](https://dietpi.com/docs/software/gaming/#box64)
* [**Moonlight (CLI) - CLI game streaming client for Sunshine and NVIDIA GameStream**](https://dietpi.com/docs/software/gaming/#moonlight-cli)
* [**Moonlight (GUI) - GUI game streaming client for Sunshine and NVIDIA GameStream**](https://dietpi.com/docs/software/gaming/#moonlight-gui)

### [Hardware Projects](https://dietpi.com/docs/software/hardware\_projects/)[¶](https://dietpi.com/docs/software/#hardware-projects) <a href="#hardware-projects" id="hardware-projects"></a>

* [**Google AIY - Voice kit “Ok, Google”!**](https://dietpi.com/docs/software/hardware\_projects/#google-aiy)
* [**Mycroft AI - Open source voice assistant**](https://dietpi.com/docs/software/hardware\_projects/#mycroft-ai)
* [**PiJuice - PiSupply UPS/battery power system**](https://dietpi.com/docs/software/hardware\_projects/#pijuice)
* [**RPi.GPIO - GPIO Interface library for RPi (python)**](https://dietpi.com/docs/software/hardware\_projects/#rpigpio)
* [**WiringPi - GPIO Interface library**](https://dietpi.com/docs/software/hardware\_projects/#wiringpi)
* [**WebIOPi - Web interface to control RPi GPIO**](https://dietpi.com/docs/software/hardware\_projects/#webiopi)
* [**Node-RED - Visual tool for wiring together hardware devices, APIs and online services**](https://dietpi.com/docs/software/hardware\_projects/#node-red)
* [**Mosquitto - Message broker that implements MQTT protocol**](https://dietpi.com/docs/software/hardware\_projects/#mosquitto)
* [**Blynk Server - iOS and Android apps to control Arduino, ESP8266, Raspberry Pi and similar microcontroller boards over the Internet**](https://dietpi.com/docs/software/hardware\_projects/#blynk-server)
* [**Audiophonics PI-SPC - Power control module for Raspberry Pi, allowing physical button power on/off**](https://dietpi.com/docs/software/hardware\_projects/#audiophonics-pi-spc)
* [**Grafana - The open platform for beautiful analytics and monitoring**](https://dietpi.com/docs/software/hardware\_projects/#grafana)

### [Home Automation](https://dietpi.com/docs/software/home\_automation/)[¶](https://dietpi.com/docs/software/#home-automation) <a href="#home-automation" id="home-automation"></a>

* [**Home Assistant - Open-source home automation platform running on Python 3**](https://dietpi.com/docs/software/home\_automation/#home-assistant)
* [**Domoticz - Multi platform Home Automation System**](https://dietpi.com/docs/software/home\_automation/#domoticz)
* [**TasmoAdmin - Administrative website for Tasmota devices**](https://dietpi.com/docs/software/home\_automation/#tasmoadmin)
* [**openHAB - Open Home Automation Bus, an open source home automation platform**](https://dietpi.com/docs/software/home\_automation/#openhab)
* [**Homebridge - Bringing HomeKit support where there is none**](https://dietpi.com/docs/software/home\_automation/#homebridge)

### [Logging Systems](https://dietpi.com/docs/software/log\_system/)[¶](https://dietpi.com/docs/software/#logging-systems) <a href="#logging-systems" id="logging-systems"></a>

* [**DietPi-RAMlog - Lightweight RAM logging**](https://dietpi.com/docs/software/log\_system/#dietpi-ramlog)
* [**Full - Full logging system with Rsyslog and Logrotate**](https://dietpi.com/docs/software/log\_system/#full-logging)

### [Media Systems](https://dietpi.com/docs/software/media/)[¶](https://dietpi.com/docs/software/#media-systems) <a href="#media-systems" id="media-systems"></a>

* [**Kodi - Media Centre / Player**](https://dietpi.com/docs/software/media/#kodi)
* [**ympd - Lightweight web Interface audio player for MPD**](https://dietpi.com/docs/software/media/#ympd)
* [**myMPD - Lightweight web Interface audio player for MPD**](https://dietpi.com/docs/software/media/#mympd)
* [**O!MPD - Feature-rich web Interface audio player for MPD**](https://dietpi.com/docs/software/media/#ompd)
* [**CAVA - Optional: Console-based audio visualizer for MPD**](https://dietpi.com/docs/software/media/#cava)
* [**Mopidy - Web Interface Music /Radio Player for MPD**](https://dietpi.com/docs/software/media/#mopidy)
* [**Airsonic-Advanced - Feature rich media streaming server with web interface**](https://dietpi.com/docs/software/media/#airsonic)
* [**Logitech Media Server - aka LMS, Squeezebox Server**](https://dietpi.com/docs/software/media/#logitech-media-server)
* [**Squeezelite - Audio player for Logitech Media Server**](https://dietpi.com/docs/software/media/#squeezelite)
* [**Shairport Sync - AirPlay audio player with multiroom sync**](https://dietpi.com/docs/software/media/#shairport-sync)
* [**ReadyMedia - (MiniDLNA) Media streaming server (DLNA, UPnP)**](https://dietpi.com/docs/software/media/#readymedia)
* [**Ampache - Web interface media streaming server**](https://dietpi.com/docs/software/media/#ampache)
* [**Emby - Web interface media streaming server**](https://dietpi.com/docs/software/media/#emby)
* [**Plex Media Server - Web interface media streaming server**](https://dietpi.com/docs/software/media/#plex-media-server)
* [**Tautulli - Monitoring and tracking tool for Plex Media Server**](https://dietpi.com/docs/software/media/#tautulli)
* [**Murmur - Mumble VoIP Server**](https://dietpi.com/docs/software/media/#murmur)
* [**Roon Bridge - Turns your device into a Roon capable audio player**](https://dietpi.com/docs/software/media/#roon-bridge)
* [**Roon Server - Turns your device into a Roon capable audio player and Roon core**](https://dietpi.com/docs/software/media/#roon-server)
* [**Roon Extension Manager - Manage extensions from within Roon**](https://dietpi.com/docs/software/media/#roon-extension-manager)
* [**NAA Daemon - Signalyst Network Audio Adaptor (NAA)**](https://dietpi.com/docs/software/media/#naa-daemon)
* [**IceCast - Shoutcast Streaming Server, including DarkIce**](https://dietpi.com/docs/software/media/#icecast)
* [**Koel - Web interface streaming server**](https://dietpi.com/docs/software/media/#koel)
* [**GMediaRender - Resource efficient UPnP/DLNA renderer**](https://dietpi.com/docs/software/media/#gmediarender)
* [**Ubooquity - Free home server for your comics and ebooks library**](https://dietpi.com/docs/software/media/#ubooquity)
* [**Komga - Free and open source comics/mangas media server with web UI**](https://dietpi.com/docs/software/media/#komga)
* [**Raspotify - Spotify Connect client**](https://dietpi.com/docs/software/media/#raspotify)
* [**Spotifyd - Lightweight FOSS Spotify client**](https://dietpi.com/docs/software/media/#spotifyd)
* [**Jellyfin - Media streaming server including live TV**](https://dietpi.com/docs/software/media/#jellyfin)
* [**Beets - Music organizer and manager**](https://dietpi.com/docs/software/media/#beets)
* [**Snapcast Server - Multi room audio server**](https://dietpi.com/docs/software/media/#snapcast-server)
* [**Snapcast Client - Multi room audio client**](https://dietpi.com/docs/software/media/#snapcast-client)
* [**Navidrome - Web interface media streaming server**](https://dietpi.com/docs/software/media/#navidrome)
* [**Kavita - Open source comics/mangas/ebooks media server**](https://dietpi.com/docs/software/media/#kavita)

### [Printing Server](https://dietpi.com/docs/software/printing/)[¶](https://dietpi.com/docs/software/#printing-server) <a href="#printing-server" id="printing-server"></a>

* [**OctoPrint - Web interface for controlling 3D printers**](https://dietpi.com/docs/software/printing/#octoprint)
* [**CUPS - Common UNIX printing system**](https://dietpi.com/docs/software/printing/#cups)

### [Remote Desktop & Remote Access](https://dietpi.com/docs/software/remote\_desktop/)[¶](https://dietpi.com/docs/software/#remote-desktop-remote-access) <a href="#remote-desktop-remote-access" id="remote-desktop-remote-access"></a>

#### [Remote Desktop](https://dietpi.com/docs/software/remote\_desktop/#remote-desktop)[¶](https://dietpi.com/docs/software/#remote-desktop) <a href="#remote-desktop" id="remote-desktop"></a>

* [**TigerVNC Server - Desktop for remote connection**](https://dietpi.com/docs/software/remote\_desktop/#tigervnc-server)
* [**RealVNC Server - Desktop for remote connection**](https://dietpi.com/docs/software/remote\_desktop/#realvnc-server)
* [**XRDP - Remote desktop server for Windows Remote Desktop Client**](https://dietpi.com/docs/software/remote\_desktop/#xrdp)
* [**NoMachine - Feature rich remote desktop connection**](https://dietpi.com/docs/software/remote\_desktop/#nomachine)

#### [Remote Access](https://dietpi.com/docs/software/remote\_desktop/#remote-access)[¶](https://dietpi.com/docs/software/#remote-access) <a href="#remote-access" id="remote-access"></a>

* [**Remote.It - Access your device over the internet**](https://dietpi.com/docs/software/remote\_desktop/#remoteit)
* [**VirtualHere - Share physically attached USB devices from your SBC over the network**](https://dietpi.com/docs/software/remote\_desktop/#virtualhere)

### [Social & Search](https://dietpi.com/docs/software/social/)[¶](https://dietpi.com/docs/software/#social-search) <a href="#social-search" id="social-search"></a>

* [**FreshRSS - A self-hosted RSS feed aggregator**](https://dietpi.com/docs/software/social/#freshrss)
* [**phpBB - Free flat-forum bulletin board software solution**](https://dietpi.com/docs/software/social/#phpbb)
* [**WordPress - Website Blog and Publishing platform**](https://dietpi.com/docs/software/social/#wordpress)
* [**Single File PHP Gallery - Host and browse your images from a web interface**](https://dietpi.com/docs/software/social/#single-file-php-gallery)
* [**Baïkal - Lightweight CalDAV + CardDAV server**](https://dietpi.com/docs/software/social/#baikal)
* [**Synapse - Decentralized communication with the Matrix protocol**](https://dietpi.com/docs/software/social/#synapse)
* [**microblog.pub - A self-hosted, single-user, ActivityPub powered microblog**](https://dietpi.com/docs/software/social/#microblogpub)
* [**MediaWiki - A collaboration and documentation platform**](https://dietpi.com/docs/software/social/#mediawiki)

### [SSH Server](https://dietpi.com/docs/software/ssh/)[¶](https://dietpi.com/docs/software/#ssh-server) <a href="#ssh-server" id="ssh-server"></a>

* [**Dropbear - Lightweight SSH Server**](https://dietpi.com/docs/software/ssh/#dropbear)
* [**OpenSSH - Feature rich SSH server with SFTP/SCP support**](https://dietpi.com/docs/software/ssh/#openssh)

### [System Security](https://dietpi.com/docs/software/system\_security/)[¶](https://dietpi.com/docs/software/#system-security) <a href="#system-security" id="system-security"></a>

* [**Let’s Encrypt - Enable HTTPS / SSL**](https://dietpi.com/docs/software/system\_security/#lets-encrypt)
* [**Fail2Ban - Protects your system from brute-force attacks**](https://dietpi.com/docs/software/system\_security/#fail2ban)

### [System Stats & Management](https://dietpi.com/docs/software/system\_stats/)[¶](https://dietpi.com/docs/software/#system-stats-management) <a href="#system-stats-management" id="system-stats-management"></a>

* [**DietPi-Dashboard - Official lightweight standalone DietPi web interface**](https://dietpi.com/docs/software/system\_stats/#dietpi-dashboard)
* [**DietPi-CloudShell - Lightweight system stats for your LCD display or monitor**](https://dietpi.com/docs/software/system\_stats/#dietpi-cloudshell)
* [**Linux Dash - Web interface system stats**](https://dietpi.com/docs/software/system\_stats/#linux-dash)
* [**phpSysInfo - Web interface system stats**](https://dietpi.com/docs/software/system\_stats/#phpsysinfo)
* [**RPi-Monitor - Web interface system stats**](https://dietpi.com/docs/software/system\_stats/#rpi-monitor)
* [**Netdata - Web interface system stats**](https://dietpi.com/docs/software/system\_stats/#netdata)
* [**Webmin - Remote system management with web interface**](https://dietpi.com/docs/software/system\_stats/#webmin)
* [**K3s - Lightweight Kubernetes**](https://dietpi.com/docs/software/system\_stats/#k3s)
* [**MicroK8s - Low-ops, minimal production Kubernetes**](https://dietpi.com/docs/software/system\_stats/#microk8s)
* [**Prometheus Node Exporter - Prometheus exporter for hardware and OS metrics**](https://dietpi.com/docs/software/system\_stats/#prometheus-node-exporter)
* [**Homer - A modern homepage for your services**](https://dietpi.com/docs/software/system\_stats/#homer)

### [VPN](https://dietpi.com/docs/software/vpn/)[¶](https://dietpi.com/docs/software/#vpn) <a href="#vpn" id="vpn"></a>

* [**OpenVPN - Easy to use, minimal hassle VPN server**](https://dietpi.com/docs/software/vpn/#openvpn)
* [**PiVPN - OpenVPN server installer & management tool**](https://dietpi.com/docs/software/vpn/#pivpn)
* [**WireGuard - An extremely simple yet fast and modern VPN**](https://dietpi.com/docs/software/vpn/#wireguard)
* [**Tailscale - Zero config VPN**](https://dietpi.com/docs/software/vpn/#tailscale)
* [**ZeroTier - Free easy to deploy cloud-hosted VPN service**](https://dietpi.com/docs/software/vpn/#zerotier)

### [Web development](https://dietpi.com/docs/software/webserver\_stack/)[¶](https://dietpi.com/docs/software/#web-development) <a href="#web-development" id="web-development"></a>

#### [Stacks for web development](https://dietpi.com/docs/software/webserver\_stack/#stacks-for-web-development)[¶](https://dietpi.com/docs/software/#stacks-for-web-development) <a href="#stacks-for-web-development" id="stacks-for-web-development"></a>

* [**LAMP web stack - Apache / MariaDB / PHP**](https://dietpi.com/docs/software/webserver\_stack/#lamp-web-stack)
* [**LASP web stack - Apache / SQLite / PHP**](https://dietpi.com/docs/software/webserver\_stack/#lasp-web-stack)
* [**LEMP web stack - Nginx / MariaDB / PHP**](https://dietpi.com/docs/software/webserver\_stack/#lemp-web-stack)
* [**LESP web stack - Nginx / SQLite / PHP**](https://dietpi.com/docs/software/webserver\_stack/#lesp-web-stack)
* [**LLMP web stack - Lighttpd / MariaDB / PHP**](https://dietpi.com/docs/software/webserver\_stack/#llmp-web-stack)
* [**LLSP web stack - Lighttpd / SQLite / PHP**](https://dietpi.com/docs/software/webserver\_stack/#llsp-web-stack)

#### [Webservers](https://dietpi.com/docs/software/webserver\_stack/#webservers)[¶](https://dietpi.com/docs/software/#webservers) <a href="#webservers" id="webservers"></a>

* [**Apache - Feature-rich webserver**](https://dietpi.com/docs/software/webserver\_stack/#apache)
* [**Nginx - High performance web server, load balancer & reverse proxy**](https://dietpi.com/docs/software/webserver\_stack/#nginx)
* [**Lighttpd - Extremely lightweight webserver**](https://dietpi.com/docs/software/webserver\_stack/#lighttpd)
* [**Tomcat8 - Apache Tomcat server**](https://dietpi.com/docs/software/webserver\_stack/#tomcat)

#### [Programming & Web development Frameworks](https://dietpi.com/docs/software/webserver\_stack/#web-development-programming-frameworks)[¶](https://dietpi.com/docs/software/#programming-web-development-frameworks) <a href="#programming-web-development-frameworks" id="programming-web-development-frameworks"></a>

* [**PHP - Scripting language especially suited to web development**](https://dietpi.com/docs/software/webserver\_stack/#php)
* [**PHP Composer** - A Dependency Manager for PHP](https://dietpi.com/docs/software/webserver\_stack/#php-composer)
* [**Flask - Micro web framework powered by Python**](https://dietpi.com/docs/software/webserver\_stack/#flask)
* [**Node.js** - JavaScript runtime designed to build scalable network applications](https://dietpi.com/docs/software/webserver\_stack/#nodejs)
